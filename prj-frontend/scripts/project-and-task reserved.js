    ...
    function loadTasks() {
        $.ajax({
            ...
            success: function (data) {
                ...
                const addedTasks = {}; // Объект для хранения уникальных задач

                data.forEach(task => {
                    // Проверка на уникальность по ID задачи
                    if (!addedTasks[task.id]) {
                        addedTasks[task.id] = true;

                        // Автоматическое изменение статуса на "Завершена", если оставшиеся дни = 0
                        if (task.days_left <= 0) {
                        ...
                        // Создание HTML для отображения задач
                        let taskHtml = `
                            <div class="task">
                                <p>Название: ${task.name}</p>
                                <p>Проект: ${projectName}</p>
                                <p>Оставшиеся дни: ${daysLeft}</p>
                                <p>Статус: ${task.status}</p>`;

                        // Проверка статуса задачи для управления возможностью редактирования и удаления
                        if (task.status !== 'Завершена') {
                            taskHtml += `
                                <div class="task-actions">
                                    <button class="btn btn-info edit-task" data-id="${task.id}">Редактировать</button>
                ...
                attachTaskActions(); // Привязываем события к кнопкам редактирования и удаления
            },
